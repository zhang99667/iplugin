...
| `html-report` | 生成独立单文件 HTML 报告；正式文档加抬头，内置代码评审、验收、选型对比、项目总结等结构模板，支持多语言离线高亮、图片/视频证据和内联 SVG；可注入离线批注审核模式，把 `AgentQuestionPack` 直接内嵌回 HTML 交给 Agent 更新，并导出无批注发布版 | "整理成 HTML 报告"、"给这个 HTML 加批注审核模式"、"按 HTML 内嵌批注更新报告"、`/html-report` |
