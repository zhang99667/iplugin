...
| `html-report` | 生成独立单文件 HTML 报告；正式文档加抬头，内置代码评审、验收、选型对比、项目总结等结构模板，代码块支持 Objective-C / Swift / C-family 等多语言离线高亮，可按需展示图片/视频证据，复杂技术图可调用 `svg-tech-diagram` 生成内联 SVG，可按需注入可编辑的离线批注审核模式并导出 Markdown/JSON 提问包和发布版 HTML | "整理成 HTML 报告"、"给这个 HTML 加批注审核模式"、`/html-report` |
