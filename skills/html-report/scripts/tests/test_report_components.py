...
class ReportComponentTest(unittest.TestCase):
    ...
    def test_annotation_launcher_remains_a_stable_sidebar_entry(self) -> None:
        ...
        self.assertIn('id="qaCopyForAgent" aria-disabled="true"', annotated)
        self.assertIn("copyForAgent.setAttribute('aria-disabled', String(unavailable))", annotated)
        self.assertIn("当前没有可交接的批注，请先添加问题或评论", annotated)
        self.assertIn("getOrCreateToast", annotated)
        self.assertIn("document.querySelector('.toast')", annotated)
        self.assertIn("resolveReportSource", annotated)
        self.assertIn("protocol === 'file:'", annotated)
