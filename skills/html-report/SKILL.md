...
- `scripts/highlight_code.py`：代码片段 HTML 转义和基础高亮脚本。报告包含代码、SQL、XML、JSON、配置片段或 diff 时必须用它生成可嵌入的 `.code-wrap` 片段；展示 unified diff 修改点时使用 `--diff-view`。
- `scripts/check_html_report.py`：生成后校验脚本。写完 HTML 后运行它检查外部 CSS/JS、裸 `<pre><code>`、`.code-wrap` 和 `tok-*`。
...
5. 报告包含代码、SQL、XML、JSON、配置片段或 diff 时，先用 `scripts/highlight_code.py` 生成高亮 HTML 片段，再嵌入报告；不要手写裸 `<pre><code>`。
6. 写 HTML 文件前读取 `references/css-template.md`，使用内嵌 CSS/JS 生成单文件 HTML。
...
- 代码高亮必须是离线单文件方案：默认使用 `scripts/highlight_code.py` 做静态预高亮，不引入 CDN，不依赖外部 CSS/JS 文件。
...
- 代码块必须先转义再高亮，使用 `scripts/highlight_code.py` 生成静态 HTML；文件位置必须尽量可跳转。
