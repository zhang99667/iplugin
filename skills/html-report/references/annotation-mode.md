...
- 自动定位失败的批注卡片提供 `重新关联`；进入临时模式后侧栏自动收起，在正文选中新位置，再点击选区气泡中的 `重新关联` 完成迁移。成功后保留原批注 `id`、类型、正文和创建时间，只更新 `blockId`、章节、选中文本及上下文锚点。
...
- 点击定位时如果原 `blockId` 已失效，只允许迁移到原文唯一命中的当前正文节点；找不到或存在多个候选时提示用户手动重新关联，不能静默失败或模糊猜测。
- 手动重新关联只允许从 `<main>` 内的合法选区提交；`Esc`、取消按钮或无效/正文外选区都只退出临时模式，不修改原批注。重新关联完成后仍需经过现有安全定位和保存校验。
