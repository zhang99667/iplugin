...
- 点击入口后出现贴近选区的小浮层，浮层只有一个 `提交` 按钮；按钮内必须显示 `Ctrl/⌘ + Enter` 快捷键标识，支持 `⌘ + Enter`（Windows/Linux 为 `Ctrl + Enter`）提交，普通 Enter 保留换行，点击浮层外侧自动关闭。
- 右键菜单作为辅助入口，支持对选中内容、本段或本节提问/批注。
- 右上角入口在从未批注的 0 条状态显示为明确的 `导出无批注版`，点击直接导出不含评论数据的发布版；如果用户刚删除或清空了最后一条批注，则显示无数量徽标的 `保存评论结果` 并保持侧栏可达，以便把空结果写回 HTML；有批注时显示 `批注 N`。
- 右侧栏列出所有批注，支持定位、编辑、复制单条、删除、`保存评论结果到 HTML`、复制 Markdown、下载 Markdown、导出无批注版、清空批注；`保存评论结果到 HTML` 必须是最明显的主按钮。
...
点击 `保存评论结果到 HTML` 后，必须生成仍可继续评论的单文件 HTML，并把当前 `AgentQuestionPack` 放进 `<head>` 中唯一的非执行节点：
...
1. 先运行 `python3 skills/html-report/scripts/check_html_report.py <html-file> --require-review-pack`。如果缺少评论包、JSON 损坏或存在多个包，必须停止修改并请用户点击 `保存评论结果到 HTML` 后提供正确文件；不能把校验失败解释成“没有批注”。
...
- `保存评论结果到 HTML` 必须在打开文件选择器前完成定位校验；任何非空评论无法在当前 `<main>` 中安全定位时停止保存，保证生成的评论包通过 `--require-review-pack`。
- 批注数据可以临时存 `localStorage`，但不能只依赖它；内嵌评论版 HTML 是默认可靠交接方式，Markdown 是兜底。
- `check_html_report.py` 必须在最终报告上通过。它会检查批注标记、保存评论结果入口、内嵌包 schema 与唯一性、安全序列化、发布版剥离、路径字段、选区缓存、单按钮浮层和快捷提交契约；接收用户已完成评论的文件时额外使用 `--require-review-pack`，防止错拿原始版或发布版。
