...
- “批注”作为工作区和交接总称，单条内容明确分为“问题”和“评论”。
- 右上角稳定打开批注侧栏；选区气泡直接提供“问题 / 评论”两个入口，右键在选中内容/本段/本节三个范围下也支持两类。
- 不保留“全部/问题/评论”类型筛选；旧提问映射为问题，旧注释和批注映射为评论。
- 侧栏主操作是“复制批注给 Agent”，复制失败提供手动 textarea；“保存批注版 HTML（备用）”不再伪装成处理完成。
- 复制批注给 Agent、复制此条和内嵌 `AgentQuestionPack` 都必须保留问题/评论类型。
