...
新图优先从这个浅色、朴素的骨架开始，按内容调整 `viewBox` 和标题。`1200 675` 只是通用横图初稿示例，不是固定尺寸；最终交付前必须用内容包围盒加外边距反推实际 `viewBox`。如果只有一行节点或单行生命周期，先把高度收紧到内容高度加上下外边距；如果内容横向没有占满 `1200px`，宽度也要同步收紧。
...
调整 `viewBox` 时，同步调整背景 `<rect>` 的 `width` / `height`。例如单行流程可用 `viewBox="0 0 1080 390"` 和 `<rect width="1080" height="390" .../>`，不要保留 1200 宽或 675 高导致周围空白。若主体节点集中在上半屏，先整体下移主体或裁短画布；不要只把说明区往下放来消耗空白。

自适应画布示例：

```text
content bbox: min=(64, 42) max=(1016, 334)
margin: left/right=48 top=42 bottom=48
final viewBox: 0 0 1048 382
平移内容: dx=-16 dy=0，使最左内容从 x=48 开始
background: <rect width="1048" height="382" fill="#FFFFFF"/>
```

如果采用非零起点，也必须让背景覆盖同一范围：

```svg
<svg xmlns="http://www.w3.org/2000/svg" viewBox="16 0 1048 382" role="img" aria-labelledby="title desc">
  <rect x="16" y="0" width="1048" height="382" fill="#FFFFFF"/>
</svg>
```
...
适合表达多个变量、条件或证据共同导致一个现象、结论或策略。

不要把 3 条以上连线直接打到同一个目标节点。更稳的结构是：

```text
多个变量节点 -> 变量池 / 归因混杂 / 汇聚层 -> 失败现象 -> 收敛策略
```

布局规则：

- 多个输入节点先连到中间汇聚节点，汇聚节点再用一条主箭头指向目标。
- 输入连线的终点要落在汇聚节点不同边或不同 y/x 通道上，不要聚集到同一个 `20px` 到 `40px` 小区域。
- 汇聚节点和目标节点之间保留至少 `52px` gutter；空间不足时缩短文案、缩小节点宽度或改成上下分层。
- 右侧输入节点到目标节点之间距离不够时，不要画斜线穿过节点；改为先向外走水平/垂直通道，再进入汇聚节点。
- 如果目标是“失败现象”，把多条边的语义写在汇聚节点中，例如“归因混杂”“证据不足”“变量池”，目标节点只承接结论。

反例：

```svg
<!-- 反例：4 条曲线终点都挤到 x=690, y=210..234，视觉上会变成一束乱线 -->
<path d="M 290 168 C 420 168, 560 200, 690 212"/>
<path d="M 290 268 C 420 268, 560 240, 690 232"/>
<path d="M 570 168 C 620 174, 660 198, 690 210"/>
<path d="M 570 268 C 620 262, 660 242, 690 234"/>
```

正例：

```svg
<!-- 正例：多输入先进入汇聚层，后面只保留单箭头 -->
<path d="M 300 160 C 390 160, 430 205, 520 205"/>
<path d="M 300 260 C 390 260, 430 235, 520 235"/>
<path d="M 520 220 L 650 220"/>
```
...
content-bbox: x=80..1120 y=40..388
final-canvas: 1200x430，四边外边距≈40..80
```
