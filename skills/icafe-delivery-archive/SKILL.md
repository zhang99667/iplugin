...
description: iCafe 需求交付材料归档助手，仅当用户明确要求“归档 iCafe 卡片材料”“整理到需求交付文件夹”“把需求描述和技术详设放一起”时触发。不要因单纯查询卡片、推断卡片号、查看需求描述或创建/更新 iCafe 卡片触发。
user_invocable: true
---
...
如果用户只是查询卡片、推断卡片号、读取需求描述、创建卡片、更新字段或改评论，应使用 iCafe 卡片助手；如果用户只要写项目复盘总结，应使用 `project-summary`。

### 需要确认

如果用户只说“这个详设对应哪个卡片”但没有要求归档或写入交付目录，先只回答卡片识别结果；需要创建目录、复制文件或覆盖同名归档文件前，必须确认用户确实要归档。
