...
生成命令会自动运行 `pivot_tool.ooxml_guard` 做 PivotTable OOXML 兼容性闸门校验。该闸门会校验 `styles.xml` 的 Normal 命名样式、样式索引、工作簿关系以及所有透视表/cache 结构，拦截 zip/openpyxl 可能放过、但 Excel 打开时会提示修复的问题；如需单独诊断已生成文件，可运行：
...
`pivot_layout.row_item_order` 是“优先排序”配置，不是过滤列表：只指定部分枚举值
时，工具会把这些值排在前面，并自动追加其余实际枚举值，避免新数据值被静默丢弃。
