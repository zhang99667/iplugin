...
- **修改 `pivot_tool/` 代码前必读 `references/ooxml_pitfalls.md`** — 列出 6 条 Excel 兼容性约束（refreshOnLoad、colFields、firstDataRow、占位符容错、属性顺序、items 仅对 axis 字段生成）。**仅在 Agent 被要求修改 `pivot_tool/*.py` 时才读**，常规生成透视表的工作流不需要加载，避免占用上下文。
