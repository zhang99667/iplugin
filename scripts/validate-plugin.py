...
HTML_REPORT_ANNOTATION_ASSETS = {
    ...
    SKILLS_DIR / "html-report" / "assets" / "annotation-mode" / "annotation.html": (
        ...
        "复制批注给 Agent",
        "问题或解释请求直接回答",
        "只有用户明确要求修改批注、报告或 HTML 时才修改同一份 HTML",
        "保存批注版 HTML（备用）",
        "导出发布版",
        'id="qaSelectionQuestionAction"',
        'id="qaSelectionQuestionLabel"',
        'id="qaSelectionAction"',
        'id="qaSelectionActionLabel"',
        'data-qa-action="question-selection" title="添加问题"',
...
HTML_REPORT_ANNOTATION_FORBIDDEN = {
    SKILLS_DIR / "html-report" / "assets" / "annotation-mode" / "annotation.html": (
        ...
        'data-qa-action="note-selection"',
        'id="qaCopyMarkdown"',
        'id="qaDownloadMarkdown"',
...
HTML_REPORT_CODE_BLOCK_ASSETS = {
    SKILLS_DIR / "html-report" / "assets" / "components" / "code-block" / "style.css": (
        ...
        'html[data-html-report-code-block-ready="true"] .copy-btn:focus-visible',
